...
BROKER_ENDPOINT = "gvkbio.vacustech.in"
PORT = 8883
BROKER_USERNAME = "Vacus"  
BROKER_PASSWORD = "vacus"  
topic = "tracking"
bufferDict = {}
...
def fetchGatewayMac():
    with open("/home/vacus/Desktop/configuration/macid.conf","r+") as fd:
            ...
            if byteBufferLength > frame_size:
                ...
                for loc in possible_locs:
                    if loc > (frame_size - 1):
                        ...
                        try:
                            with open("/home/vacus/Desktop/configuration/socialParams.json","r+") as fd:
...
def processDataFrame(data_array):
    ...
    for pair in split_data_array:
        if pair[1]:
            ...
            db_health_payload.append({
                "id": "5a-c2-15-a1" + "-" + "{0:0{1}x}".format(pair[0], 2) + "-" + "{0:0{1}x}".format(pair[1], 2),
                "bt": int(pair[10]),
                "type": 1,
                "timestamp": int(round(time.time()*1000))
            })
    ...
    if len(bufferDict)!=0:
        ...
        for key,val in bufferDict.items():
            ...
            temp = {}
            jsonTemp = val[0]
            temp ["id1"] = "5a-c2-15-a1" + "-" + jsonTemp["tag1"]
            temp ["id2"] = "5a-c2-15-a1" + "-" + jsonTemp["tag2"]
...
dbClient.tls_set("/home/vacus/Desktop/ssl/chained+root.crt")
